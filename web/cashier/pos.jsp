...
<%
    ...
    // Store cashier name for JavaScript
    String jsCashierName = cashierName != null ? cashierName.replace("'", "\\'") : "Cashier";
...
<head>
    ...
    <title>Point of Sale — Mpeoa Supermarket ERP</title>
...
<aside class="sidebar" id="sidebar">
    ...
    <nav class="sidebar-nav">
        ...
        <a href="<%= contextPath %>/cashier/dashboard.jsp" class="nav-item-link">
            <i class="bi bi-grid-1x2-fill"></i> Dashboard
        </a>
        
        <div class="nav-section-label">Operations</div>
        <a href="<%= contextPath %>/cashier/pos.jsp" class="nav-item-link active">
            <i class="bi bi-cash-register"></i> Point of Sale
        </a>
        <a href="<%= contextPath %>/cashier/my-sales.jsp" class="nav-item-link">
            <i class="bi bi-receipt"></i> My Sales
        </a>

        <div class="nav-section-label">Products</div>
        <a href="<%= contextPath %>/cashier/products.jsp" class="nav-item-link">
...
<script>
    ...
    function addToCartFromCard(card) {
        let id = parseInt(card.dataset.id, 10);
        let name = card.dataset.name || '';
        let code = card.dataset.code || '';
        let category = card.dataset.category || '';
        let price = parseFloat(card.dataset.price) || 0;
        let costPrice = parseFloat(card.dataset.cost) || 0;
        let stock = parseInt(card.dataset.stock, 10) || 0;
    ...
    function updateCartDisplay() {
        ...
        for (let i = 0; i < cart.length; i++) {
            ...
            let itemCost = item.costPrice * item.quantity;
            let itemProfit = itemTotal - itemCost;
            subtotal += itemTotal;
            totalCost += itemCost;
        ...
        let grossProfit = subtotal - totalCost;
    ...
    function generateReceipt(invoiceNumber, paymentMethod) {
        ...
        receiptHtml += '</tbody></table>' +
            ...
            '</div>' +
            '<div class="receipt-divider"></div>' +
            '<p class="text-center mb-0">Thank you for shopping at Mpeoa!</p>' +
            '<p class="text-center" style="font-size: 0.65rem;">Goods sold are not returnable or refundable</p>' +
