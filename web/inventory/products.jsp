<%-- web/admin/products.jsp --%>
<%@ page language="java" contentType="text/html; charset=UTF-8" pageEncoding="UTF-8"%>
<%@ page import="mpeoa.models.User, mpeoa.dao.ProductDAO, mpeoa.models.Product, mpeoa.dao.CategoryDAO, mpeoa.models.Category, java.util.*, java.text.*, java.math.BigDecimal" %>
<%
    ...
    if (products != null) {
        for (Product p : products) {
            ...
            totalInventoryValue = totalInventoryValue.add(p.getCostPrice().multiply(new BigDecimal(p.getStockQuantity())));
        }
    }
...
<head>
    ...
    <title>Product Management — Mpeoa Supermarket ERP</title>
    ...
    <style>
        ...
        *, *::before, *::after { box-sizing: border-box; }
        ...
        .form-buttons { display: flex; gap: 12px; margin-top: 10px; }
        ...
        .btn-warning-custom:hover { background: #b8860b; color: white; }
        ...
        .alert-custom { padding: 12px 16px; border-radius: 10px; margin-bottom: 20px; display: flex; align-items: center; gap: 10px; }
...
<aside class="sidebar" id="sidebar">
    ...
    <nav class="sidebar-nav">
        ...
        <a href="${pageContext.request.contextPath}/admin/dashboard" class="nav-item-link">
        ...
        <a href="${pageContext.request.contextPath}/admin/products.jsp" class="nav-item-link active">
    ...
    <main class="page-body">
        <div class="page-header">
            <div>
                ...
                <p class="text-muted mt-2">Manage your product catalog, pricing, and stock levels</p>
            </div>
            <div class="d-flex gap-2">
                <a href="${pageContext.request.contextPath}/admin/categories.jsp" class="btn-sm-outline">
        ...
        <div class="row g-3 mb-4">
            ...
            <div class="col-md-3 col-6">
                <div class="stat-card animate-fade">
                    ...
                    <div class="stat-value">R <%= df.format(totalInventoryValue) %></div>
                ...
                <form action="${pageContext.request.contextPath}/admin/product-add" method="post">
                    <div class="form-row">
                        <div class="form-group">
                            ...
                            <input type="text" name="productCode" class="form-control" placeholder="e.g., P001" required>
                        </div>
                        <div class="form-group">
                        ...
                        <div class="form-group">
                            ...
                            <select name="categoryId" class="form-select" required>
                                ...
                                <%      }
                                        ...
                                        <option value="">No categories available - <a href="${pageContext.request.contextPath}/admin/categories.jsp">Add categories first</a></option>
                                <% } %>
                            </select>
                        </div>
                    </div>
                    <div class="form-row">
                        <div class="form-group">
                            ...
                            <input type="number" step="0.01" name="unitPrice" class="form-control" placeholder="0.00" required>
                        </div>
                        <div class="form-group">
                        ...
                        <div class="form-group">
                            ...
                            <input type="number" name="initialStock" class="form-control" placeholder="0" value="0">
                        </div>
                        <div class="form-group">
                            <label>Reorder Level</label>
                            <input type="number" name="reorderLevel" class="form-control" placeholder="10" value="10">
                        </div>
                    </div>
                    <div class="form-row">
                        <div class="form-group">
                    ...
                    <div class="form-buttons">
                        <button type="submit" class="btn-gold"><i class="bi bi-check-lg"></i> Add Product</button>
        ...
        <div class="content-card">
            <div class="card-header-bar">
                <h2 class="card-header-title">
                    <i class="bi bi-box-seam-fill"></i> Product Catalog
                </h2>
                        ...
                        <% if (products == null || products.isEmpty()) { %>
                            <tr class="empty-state-row">
                                <td colspan="8" class="empty-state">
                                    ...
                                    <a href="${pageContext.request.contextPath}/admin/categories.jsp" class="btn-sm-outline mt-2">
                                        <i class="bi bi-tags-fill"></i> Create Categories First
                                    </a>
                                 </td>
                             </tr>
                        <% } else { 
                            for (Product product : products) { 
                                ...
                                if (product.getCostPrice() != null && product.getCostPrice().compareTo(BigDecimal.ZERO) > 0) {
                                    profitMargin = product.getUnitPrice().subtract(product.getCostPrice())
                                        .divide(product.getCostPrice(), 4, BigDecimal.ROUND_HALF_UP)
                                        .multiply(new BigDecimal(100));
                                }
                                
                                if (stockQty <= 0) {
                                    stockStatus = "Out of Stock";
                                    stockBadge = "badge-out-stock";
                                } else if (stockQty <= reorderLvl) {
                                    stockStatus = "Low Stock";
                                    stockBadge = "badge-low-stock";
                                } else {
                                    stockStatus = "In Stock";
                                    stockBadge = "badge-in-stock";
                                }
                        %>
                            <tr>
                                ...
                                <td><%= product.getCategoryName() != null ? product.getCategoryName() : "Uncategorized" %></td>
                                <td class="fw-bold">R <%= df.format(product.getUnitPrice() != null ? product.getUnitPrice() : BigDecimal.ZERO) %></td>
                                <td>R <%= df.format(product.getCostPrice() != null ? product.getCostPrice() : BigDecimal.ZERO) %></td>
                                <td><span class="<%= stockBadge %>"><%= stockQty %> units</span></td>
                                <td><span class="<%= stockBadge %>"><%= stockStatus %></span></td>
                                <td>
                                    <div class="d-flex gap-2">
                                        <button class="btn-warning-custom" onclick='editProduct(<%= product.getProductId() %>, "<%= product.getProductName() != null ? product.getProductName().replace("\"", "\\\"") : "" %>", "<%= product.getProductCode() != null ? product.getProductCode() : "" %>", <%= product.getUnitPrice() != null ? product.getUnitPrice() : BigDecimal.ZERO %>, <%= product.getCostPrice() != null ? product.getCostPrice() : BigDecimal.ZERO %>, "<%= product.getDescription() != null ? product.getDescription().replace("\"", "\\\"") : "" %>")'>
                                            <i class="bi bi-pencil"></i> Edit
                                        </button>
                                        <button class="btn-danger-custom" onclick="confirmDelete(<%= product.getProductId() %>, '<%= product.getProductName() != null ? product.getProductName().replace("'", "\\'") : "" %>')">
        ...
        <div class="content-card">
            <div class="card-header-bar">
                <h2 class="card-header-title">
                    <i class="bi bi-info-circle-fill"></i> Product Management Guidelines
                </h2>
            </div>
                ...
                <div class="row">
                    <div class="col-md-6">
                        <ul style="padding-left: 20px; color: var(--text-muted); font-size: 0.85rem; line-height: 1.8;">
                            <li><i class="bi bi-hash"></i> Product codes should be unique and follow a consistent format (e.g., P001, P002)</li>
                            <li><i class="bi bi-currency-exchange"></i> Selling price determines the customer price, cost price is your purchase cost</li>
                            <li><i class="bi bi-box-seam"></i> Reorder level triggers low stock alerts when stock falls below this number</li>
                            <li><i class="bi bi-tags"></i> Products must belong to a category - create categories first if none exist</li>
                        </ul>
                    </div>
                    <div class="col-md-6">
                        <ul style="padding-left: 20px; color: var(--text-muted); font-size: 0.85rem; line-height: 1.8;">
                            <li><i class="bi bi-pencil"></i> Click "Edit" to modify product details or update pricing</li>
                            <li><i class="bi bi-trash"></i> Deleting a product will also remove it from inventory records</li>
                            <li><i class="bi bi-graph-up"></i> Regular price reviews help maintain healthy profit margins</li>
                            <li><i class="bi bi-envelope-fill"></i> Low stock alerts help you reorder before running out</li>
                        </ul>
                    </div>
                </div>
            </div>
    ...
    <div class="modal-dialog">
        <div class="modal-content" style="border-radius: var(--radius);">
            <div class="modal-header" style="background: var(--cream); border-bottom: 1px solid var(--cream-border);">
                <h5 class="modal-title" style="font-family: var(--font-display);"><i class="bi bi-pencil-square"></i> Edit Product</h5>
            ...
            <form action="${pageContext.request.contextPath}/admin/product-edit" method="post">
                ...
                <div class="modal-body">
                    <div class="form-group">
                        <label>Product Code</label>
                        <input type="text" name="productCode" id="editProductCode" class="form-control" readonly style="background: var(--cream);">
                    </div>
                    <div class="form-group">
                        <label>Product Name</label>
                    ...
                    <div class="form-row" style="display: flex; gap: 15px;">
                        <div class="form-group" style="flex: 1;">
                            <label>Selling Price (R)</label>
                            <input type="number" step="0.01" name="unitPrice" id="editUnitPrice" class="form-control" required>
                        </div>
                        <div class="form-group" style="flex: 1;">
                            <label>Cost Price (R)</label>
                            <input type="number" step="0.01" name="costPrice" id="editCostPrice" class="form-control" required>
                        </div>
                    </div>
                    <div class="form-group">
                        <label>Description</label>
    ...
    <div class="modal-dialog">
        <div class="modal-content" style="border-radius: var(--radius);">
            ...
            <div class="modal-body">
                ...
                <p class="text-muted" style="font-size: 0.8rem;">This action cannot be undone and will remove the product from inventory.</p>
            </div>
            <div class="modal-footer" style="background: var(--cream); border-top: 1px solid var(--cream-border);">
                <form action="${pageContext.request.contextPath}/admin/product-delete" method="post">
                    <input type="hidden" name="productId" id="deleteProductId">
                    <button type="button" class="btn-outline" data-bs-dismiss="modal">Cancel</button>
                    <button type="submit" class="btn-danger-custom">Delete Product</button>
                </form>
            </div>
        </div>
    </div>
...
<script>
    ...
    function editProduct(id, name, code, unitPrice, costPrice, description) {
