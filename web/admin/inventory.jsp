...
<head>
    ...
    <title>Inventory Management — Mpeoa Supermarket ERP</title>
...
<aside class="sidebar" id="sidebar">
    ...
    <nav class="sidebar-nav">
        ...
        <a href="${pageContext.request.contextPath}/admin/inventory.jsp" class="nav-item-link active">
    ...
    <main class="page-body">
        <div class="page-header">
            <div>
                ...
                <p class="text-muted mt-2">Monitor and manage your product stock levels</p>
            </div>
            <button class="btn-sm-outline" onclick="openStockAdjustModal()">
                <i class="bi bi-plus-circle-fill"></i> Bulk Stock Adjustment
            </button>
        </div>
        ...
        <% if (lowStockProducts != null && !lowStockProducts.isEmpty()) { %>
            <div class="alert-custom alert-warning animate-fade">
                <i class="bi bi-exclamation-triangle-fill"></i>
                <span><strong><%= lowStockProducts.size() %></strong> product(s) are running low on stock. Please reorder soon!</span>
            </div>
        <% } %>
<!-- Stock Levels Table -->
<div class="content-card">
    <div class="card-header-bar">
        <h2 class="card-header-title">
            <i class="bi bi-archive-fill"></i> Current Stock Levels
        </h2>
        <span class="btn-sm-outline">Total Products: <%= (products != null) ? products.size() : 0 %></span>
    </div>
    <div class="table-responsive">
            ...
            <tbody>
                ...
                %>
                    <tr>
                        <td colspan="7" style="text-align: center; padding: 40px;">
                            <i class="bi bi-archive" style="font-size: 3rem; opacity: 0.3; display: block; margin-bottom: 15px;"></i>
                ...
                %>
                    <tr>
                        ...
                        <td><%= productName %></td>
                        <td><%= categoryName %></td>
                        <td class="fw-bold <%= stockColor %>"><%= stockQty %> units</td>
                        <td><%= reorderLvl %> units</td>
                        <td><span class="<%= stockBadge %>"><%= stockStatus %></span></td>
                        <td>
                            <div class="stock-update-form">
                                <input type="number" id="stock_<%= product.getProductId() %>" class="stock-input" placeholder="Qty" value="1" min="1">
                                <button class="btn-sm-success" onclick="updateStock(<%= product.getProductId() %>, '<%= productName.replace("'", "\\'") %>', 'add')">
                                    <i class="bi bi-plus"></i> Add
                                </button>
                                <button class="btn-danger-custom" onclick="updateStock(<%= product.getProductId() %>, '<%= productName.replace("'", "\\'") %>', 'remove')">
        ...
        <div class="content-card">
            <div class="card-header-bar">
                <h2 class="card-header-title">
                    <i class="bi bi-info-circle-fill"></i> Inventory Management Guidelines
                </h2>
            </div>
            <div style="padding: 20px;">
                <ul style="padding-left: 20px; color: var(--text-muted); font-size: 0.85rem; line-height: 1.8;">
                    <li><i class="bi bi-exclamation-triangle-fill me-1"></i> Products marked as <strong class="text-warning">Low Stock</strong> are below their reorder level</li>
                    <li><i class="bi bi-plus-circle-fill me-1"></i> Use the <strong>Add (+)</strong> button to increase stock when new shipments arrive</li>
                    <li><i class="bi bi-dash-circle-fill me-1"></i> Use the <strong>Remove (-)</strong> button for damaged or expired products</li>
                    <li><i class="bi bi-calculator-fill me-1"></i> Stock is automatically updated when sales are processed through POS</li>
                    <li><i class="bi bi-graph-up me-1"></i> Regular inventory audits help maintain accurate stock levels</li>
                    <li><i class="bi bi-envelope-fill me-1"></i> Reorder alerts are sent when stock reaches reorder level</li>
                </ul>
            </div>
        </div>
    </main>
    ...
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" style="font-family: var(--font-display);"><i class="bi bi-sliders2"></i> Bulk Stock Adjustment</h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
            </div>
            <div class="modal-body">
                <p>This feature allows you to update stock for multiple products at once.</p>
                <div class="alert-custom alert-warning mt-2" style="margin-bottom: 0;">
                    <i class="bi bi-info-circle-fill"></i>
                    <span>Coming soon: Bulk stock adjustment interface</span>
                </div>
            </div>
...
<script>
    ...
    function updateStock(productId, productName, action) {
        var quantityInput = document.getElementById('stock_' + productId);
        ...
        var confirmMsg = 'Are you sure you want to ' + actionText + ' ' + quantity + ' unit(s) of "' + productName + '"?';
        
        if (confirm(confirmMsg)) {
            // Here you would make an AJAX call to your servlet
            // For now, showing an alert
            alert('Stock update for ' + productName + ': ' + actionText + ' ' + quantity + ' units');
