...
<%@ page import="mpeoa.models.User, mpeoa.dao.SaleDAO, mpeoa.dao.ExpenseDAO, mpeoa.dao.ProductDAO, mpeoa.dao.SupplierDAO, java.util.*, java.text.*, java.math.BigDecimal, java.sql.*" %>
<%
    ...
    List<mpeoa.models.Product> topProducts = new ArrayList<>();
    
    if ("sales".equals(reportType)) {
        totalSales = saleDAO.getTotalSalesByDateRange(startDate, endDate);
        totalTransactions = saleDAO.getSaleCount();
        salesData = saleDAO.getDailySalesForPeriod(startDate, endDate);
    } else if ("expenses".equals(reportType)) {
        totalExpenses = expenseDAO.getTotalExpensesByDateRange(startDate, endDate);
        expenseData = expenseDAO.getExpensesByCategoryForPeriod(startDate, endDate);
    } else if ("profit".equals(reportType)) {
        totalSales = saleDAO.getTotalSalesByDateRange(startDate, endDate);
        totalExpenses = expenseDAO.getTotalExpensesByDateRange(startDate, endDate);
        netProfit = totalSales.subtract(totalExpenses);
    } else if ("products".equals(reportType)) {
        topProducts = productDAO.getTopSellingProducts(10);
    }
    ...
    if ("sales".equals(reportType) && salesData != null && !salesData.isEmpty()) {
        for (int i = 0; i < salesData.size(); i++) {
            Object[] row = salesData.get(i);
            chartLabels.append("'").append(row[0].toString()).append("'");
            chartValues.append(row[1].toString());
    ...
    if ("expenses".equals(reportType) && expenseData != null && !expenseData.isEmpty()) {
        for (int i = 0; i < expenseData.size(); i++) {
            Object[] row = expenseData.get(i);
            chartLabels.append("'").append(row[0].toString()).append("'");
            chartValues.append(row[1].toString());
            if (i < expenseData.size() - 1) {
                chartLabels.append(",");
                chartValues.append(",");
            }
        }
...
<head>
    ...
    <title>Reports — Mpeoa Supermarket ERP</title>
    ...
    <style>
        ...
        *, *::before, *::after { box-sizing: border-box; }
        ...
        .report-tab:hover { border-color: var(--gold); background: var(--gold-pale); }
        .report-tab.active { background: var(--gold); color: white; border-color: var(--gold); }
...
<aside class="sidebar" id="sidebar">
    ...
    <nav class="sidebar-nav">
        ...
        <a href="${pageContext.request.contextPath}/admin/reports.jsp" class="nav-item-link active">
...
<div class="main-wrap">
    ...
    <main class="page-body">
        <div class="page-header">
            <div>
                <p class="page-eyebrow">Financial & Operational Reports</p>
                <h1 class="page-heading">Reports</h1>
                <p class="text-muted mt-2">Generate and export business reports</p>
            </div>
        ...
        <div class="report-tabs">
            <a href="?type=sales&period=<%= period %>" class="report-tab <%= "sales".equals(reportType) ? "active" : "" %>">
                <i class="bi bi-graph-up"></i> Sales Report
            </a>
            <a href="?type=expenses&period=<%= period %>" class="report-tab <%= "expenses".equals(reportType) ? "active" : "" %>">
                <i class="bi bi-wallet2"></i> Expenses Report
            </a>
            <a href="?type=profit&period=<%= period %>" class="report-tab <%= "profit".equals(reportType) ? "active" : "" %>">
                <i class="bi bi-trophy"></i> Profit & Loss
            </a>
            <a href="?type=products&period=<%= period %>" class="report-tab <%= "products".equals(reportType) ? "active" : "" %>">
                <i class="bi bi-box-seam-fill"></i> Product Performance
            </a>
        </div>
        
        <!-- Filter Bar -->
        <div class="filter-bar">
            <a href="?type=<%= reportType %>&period=week" class="filter-btn <%= "week".equals(period) ? "active" : "" %>">Last 7 Days</a>
            <a href="?type=<%= reportType %>&period=month" class="filter-btn <%= "month".equals(period) ? "active" : "" %>">Last 30 Days</a>
            <a href="?type=<%= reportType %>&period=quarter" class="filter-btn <%= "quarter".equals(period) ? "active" : "" %>">Last 3 Months</a>
            <a href="?type=<%= reportType %>&period=year" class="filter-btn <%= "year".equals(period) ? "active" : "" %>">Last Year</a>
        </div>
        ...
        <% } else if ("expenses".equals(reportType)) { %>
            ...
            <div class="row g-3 mb-4">
                ...
                <div class="col-md-6">
                    <div class="stat-card animate-fade">
                        ...
                        <div class="stat-value"><%= expenseData != null ? expenseData.size() : 0 %></div>
            ...
            <div class="content-card">
                <div class="card-header-bar">
                    <h2 class="card-header-title">
                        <i class="bi bi-pie-chart-fill"></i> Expenses by Category
                    </h2>
                </div>
                <div class="chart-container">
                    <canvas id="reportChart"></canvas>
        ...
        <% } else if ("profit".equals(reportType)) { %>
            ...
            <div class="content-card">
                <div class="card-header-bar">
                    <h2 class="card-header-title">
                        <i class="bi bi-pie-chart-fill"></i> Revenue vs Expenses
                    </h2>
                </div>
                <div class="chart-container">
            ...
            <div class="content-card">
                <div class="card-header-bar">
                    <h2 class="card-header-title">
                        <i class="bi bi-calculator-fill"></i> Profit & Loss Summary
                    </h2>
                </div>
                <div class="card-body">
                    <table class="data-table">
                        <tbody>
                            <tr>
                                ...
                                <td class="text-end fw-bold">R <%= df.format(totalSales) %></td>
                             </tr>
                            <tr>
                                <td><strong>Total Expenses</strong></td>
                                <td class="text-end text-danger">R <%= df.format(totalExpenses) %></td>
                             </tr>
                            <tr style="border-top: 2px solid var(--gold);">
                                <td><strong>Net Profit / (Loss)</strong></td>
                                <td class="text-end fw-bold" style="color: <%= netProfit.compareTo(BigDecimal.ZERO) >= 0 ? "var(--success)" : "var(--danger)" %>;">
                                    R <%= df.format(netProfit) %>
                                </td>
                             </tr>
                            <tr>
                                <td>Profit Margin</td>
                                <td class="text-end">
                                    <%= totalSales.compareTo(BigDecimal.ZERO) > 0 ? pf.format(netProfit.divide(totalSales, 4, BigDecimal.ROUND_HALF_UP).multiply(new BigDecimal(100))) : "0" %>%
                                </td>
                             </tr>
                        </tbody>
                    </table>
        ...
        <% } else if ("products".equals(reportType)) { %>
            ...
            <div class="content-card">
                <div class="card-header-bar">
                    <h2 class="card-header-title">
                        <i class="bi bi-box-seam-fill"></i> Top Selling Products
                    </h2>
                </div>
                        ...
                        <tbody>
                            ...
                            %>
                                <tr>
                                    ...
                                    <td>R <%= df.format(product.getUnitPrice()) %></td>
                                    <td><%= product.getStockQuantity() %></td>
                                    <td>R <%= df.format(revenue) %></td>
                                </tr>
                            <% } } else { %>
                                <tr><td colspan="6" class="text-center">No product sales data available</td></tr>
                            <% } %>
                        </tbody>
                    </table>
                </div>
        ...
        <!-- Report Period Note -->
        <div class="alert-custom alert-info" style="background: var(--info-pale); border: 1px solid rgba(23,162,184,0.25); color: var(--info); padding: 12px 16px; border-radius: 10px; display: flex; align-items: center; gap: 10px;">
            <i class="bi bi-info-circle-fill"></i>
            <span>Report period: <strong><%= startDate %> to <%= endDate %></strong>. Use the filters above to change the date range.</span>
        </div>
    </main>
</div>
...
<script>
    ...
    <% if ("sales".equals(reportType) || "expenses".equals(reportType)) { %>
        ...
        new Chart(ctx, {
            ...
            data: {
                ...
                datasets: [{
                    label: '<%= "sales".equals(reportType) ? "Sales (R)" : "Amount (R)" %>',
                    data: [<%= chartValues.toString() %>],
                    backgroundColor: 'rgba(200,146,58,0.5)',
                    borderColor: '#C8923A',
                    borderWidth: 1
                }]
            },
    ...
    <% if ("profit".equals(reportType)) { %>
        const pieCtx = document.getElementById('profitChart').getContext('2d');
        new Chart(pieCtx, {
            type: 'doughnut',
            data: {
